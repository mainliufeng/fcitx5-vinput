...
所以长语音不整段丢给离线模型，而是**切成不超过 30 秒的片段分别精修，再把文本接起来**：

- **切点选在静音处**：在名义边界前后 3 秒内找能量最低的 200 ms 窗口作为切点，避免从词中间切开。
- **每段一个独立会话**：离线会话是单次使用的（`Finish()` 之后封口），底层的 recognizer 复用。
- **任一段失败就整体放弃**：中间缺一段会得到半句话，所以宁可退回流式结果也不提交残缺文本。

验证（53 秒音频，同一句中文重复 9 遍）：

```
second pass split 847503 samples into 2 chunks
VAD trimmed 457600 -> 395424 samples      ← 第 1 段
VAD trimmed 389903 -> 319616 samples      ← 第 2 段
second pass replaced the streaming result chunks=2
```

切缝核对：两遍都在原文里数到 9 次，**没有丢字、没有重复**。

另外整个第二遍仍包在 `try/catch` 里：任何抛出（不只是返回 false）都静默保留流式结果——这是"二级精修绝不丢句子"这条保证的落点。
...
### CPU 占用：`num_threads` 打开多少很重要

各阶段实测（0.2 s 采样，CPU 已归一化到 20 线程，即 100% = 20 核满载）：

| 阶段 | CPU 均值 | CPU 峰值 | 内存 RSS | 网络 |
|---|---|---|---|---|
| 空闲（模型已载入） | 0.0% | 0.0% | 564 MB | 0 |
| 按住说话（流式解码） | **0.3%** | **1.3%** | 555 MB | **0** |
| 松手后（精修 + LLM） | ~1% | 3.3% | 630 MB | 1 次 |

> **说话期间 TCP 连接数为 0** —— ASR 完全本地，零网络。只有松手后那一瞬会连一次 LLM。

#### 为什么曾经高 60 倍（已修）

上游的流式后端曾用运行时默认值覆盖模型自己声明的线程数：

```cpp
// 修复前：runtime_default 是硬编码的 4，永远大于 0
int ChooseNumThreads(model_cfg, runtime_default) {
  if (runtime_default > 0) return runtime_default;   // → 永远返回 4
  return JsonInt(model_cfg, "num_threads", 1);        // → 模型写的 1 被忽略
}
```

而注册表里**每一个模型**（不论流式还是离线）都显式写了 `num_threads: 1`。对这个小尺寸 int8 transducer，
onnxruntime 的线程同步开销远大于并行收益：

| num_threads | 墙钟（44.4 s 音频） | CPU 消耗 | 相对成本 |
|---|---|---|---|
| **1** | **2.09 s** | **2.06 CPU秒** | **1.0×** |
| 2 | 1.69 s | 6.70 CPU秒 | 3.3× |
| 4（修复前的实际值） | 2.20 s | 21.84 CPU秒 | **10.6×** |
| 8 | 5.48 s | 73.72 CPU秒 | 35.8× |

**线程越多又慢又贵。** 现在改为模型声明优先，实测说话期间 CPU 从均值 18.2% / 峰值 44.8%
降到 **0.3% / 1.3%**（约 60 倍），识别结果不变。


作为对照，热词（强制 `modified_beam_search`）只多花约 19%：

| 解码方式 | 墙钟 | CPU 消耗 |
|---|---|---|
| `greedy_search`（无热词） | 2.08 s | 2.04 CPU秒 |
| `modified_beam_search`（有热词，beam=4） | 2.48 s | 2.44 CPU秒 |
